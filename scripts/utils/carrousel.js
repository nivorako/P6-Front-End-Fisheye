...
function displayCarrousel(media, photographer) {
    ...
    leftArrow.classList.add("fas", "fa-chevron-left", "tab", "arrow")

    const rightArrow = document.createElement("i")
    rightArrow.setAttribute("tabindex", "1")
    rightArrow.classList.add("fas", "fa-chevron-right", "tab", "arrow")
    ...
    for (let i = 0; i < l; i++) {
        ...
        if (media[i].hasOwnProperty("video")) {
            ...
            carrouselImg.setAttribute(
                "src",
                `./assets/Sample Photos/${name}/${media[i].image}`
            )
...
function carrouselKeydownFunction() {
    ...
    const carroselBtnClose = document.querySelector(".fa-times")
    let step = 0
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Tab" || e.keyCode === 9) {
            if (e.shiftKey) {
                ...
                carrosselElts[carrosselElts.length - 1]
            ) {
                e.preventDefault()
