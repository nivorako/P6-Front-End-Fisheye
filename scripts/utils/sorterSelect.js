...
export function displaySelectedItem(sorted, media, photographer) {
    ...
    if (mediaWrapper.firstElementChild) {
        ...
        const focusablePhotographerEltsString =
            "[href], [tabindex]:not([tabindex=\"-1\"], button:not([disabled])"

        const focusablePhotographerElts = photographerBody.querySelectorAll(
...
function select(data, orderBy) {
    if (orderBy === "likes") {
        ...
        data.sort((a, b) => {
            ...
            return (
                new Date(dateB[0], dateB[1], dateB[2]).getTime() -
                new Date(dateA[0], dateA[1], dateA[2]).getTime()
            )
        })
        return data
