...
async function init() {
    ...
    if (photographerId) {
        ...
        const foundPhotographers = photographers.filter(
            (x) => x.id === parseInt(photographerId, 10)
        )
        ...
        btnPlay.addEventListener("click", () => {
            ...
            modalForm.addEventListener("keydown", (e) => {
                ...
                const lastmodalFocusElt =
                    modalFocusElts[modalFocusEltsLength - 1]
                ...
                if (
                    document.activeElement === lastmodalFocusElt &&
        ...
        // SORTER
        const foundPhotographerMedia = media.filter(
            (x) => x.photographerId === parseInt(photographerId, 10)
        )
        ...
        // set likes elt
        const sorterSelected = document.querySelector(
            ".sorter__selectedText"
        ).innerHTML
        displaySelectedItem(
            sorterSelected,
            foundPhotographerMedia,
            foundPhotographer
        )
    } else {
        ...
        const focusablePhotographerEltsString =
            "a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type='text']:not([disabled]), input[type='radio']:not([disabled]), input[type='checkbox']:not([disabled]), select:not([disabled])"
        const focusablePhotographerElts = photographersBody.querySelectorAll(
            focusablePhotographerEltsString
        )
        const focusablePhotographerEltsLength = focusablePhotographerElts.length

        const firstFocusable = focusablePhotographerElts[0]
        const lastFocusable =
            focusablePhotographerElts[focusablePhotographerEltsLength - 1]
